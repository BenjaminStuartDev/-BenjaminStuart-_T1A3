...
describe '#get_user_input' do
  ...
  it "Reprompts user if confirmation is 'N'" do
    allow(STDIN).to receive(:gets).and_return('Blues Cafe', 'N', 'Reds Cafe', 'Y')
    expect(get_user_input('')).to eq('Reds Cafe')
  end
  it "Does not reprompt the user if confirmation is 'Y" do
    allow(STDIN).to receive(:gets).and_return('Blues Cafe', 'Y')
    expect(get_user_input('')).to eq('Blues Cafe')
  end
  it 'Allows for a proc to be passed in that handles an error and allows for user to retry' do
    allow(STDIN).to receive(:gets).and_return('', 'Reds Cafe', 'Y')
...
describe '#get_confirmation' do
  ...
  it 'reprompts user if InvalidInputError is raised and attempts retry' do
    allow(STDIN).to receive(:gets).and_return('J', ' ', '!', 'Y')
    expect(get_confirmation('')).to eq('Y')
  end
end
